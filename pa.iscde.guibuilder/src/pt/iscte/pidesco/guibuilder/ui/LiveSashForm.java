 ...
 *     
 * Downloaded from: http://www.novocode.com/swt/LiveSashForm.java
 *******************************************************************************/
...
/**
 * Like a regular SashForm, the LiveSashForm lays out its children in a Row or
 * Column arrangement (as specified by the orientation) and places a Sash
 * between the children. One child may be maximized to occupy the entire size of
 * the LiveSashForm. The relative sizes of the children may be specified using
 * weights.
 * <p>
 * LiveSashForm supports the following additional features:
 * <ul>
 * <li>Unless the LiveSashForm is created with the NO_LIVE_UPDATE style, the
 * bounds of the children are updated in real time when a Sash is being dragged.
 * </li>
 * <li>Borders can be assigned to the individual child widgets by calling
 * <code>setChildBorder()</code>. This has the same effect as placing the child
 * widgets in FramedComposites except the borders become part of the adjacent
 * sashes, thus making the draggable part bigger without consuming additional
...
public class LiveSashForm extends Composite {
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		int sashwidth = sashes.length > 0 ? sashWidth + sashes[0].getBorderWidth() * 2 : sashWidth;
		int width = 0;
		int height = 0;
	 ...
	 * control occupies. The weights are returned in order of the creation of
	...
	private void onPaint(Event e) {
		if (maxControl != null && !maxControl.isDisposed()) {
			for (int i = 0; i < controls.length; i++) {
				if (controls[i] == maxControl)
					drawBorderAround(controls[i], e.gc);
			}
		} else {
			for (int i = 0; i < controls.length; i++)
				drawBorderAround(controls[i], e.gc);
	 ...
	 * orientation) or total height (if SashForm has Vertical orientation) each
	 * control will occupy. The weights must be positive values and there must
	 ...
	 *                <li>ERROR_THREAD_INVALID_ACCESS - if not called from the
	 *                thread that created the receiver</li>
	 *                <li>ERROR_INVALID_ARGUMENT - if the weights value is null
	 *                or of incorrect length (must match the number of children)
	 *                </li>
	 *                </ul>
	 */
	 ...
	 * 
	 * @param child
	 *            The child control for which to set the border.
	 * @param shadow
	 *            One of the SWT shadow constants SHADOW_IN, SHADOW_OUT,
	...
	public void setChildBorder(Control child, int shadow) {
		...
		if (shadow == SWT.SHADOW_NONE)
			...
			child.setData(CHILD_SHADOW, new Integer(shadow));
		layout();
		redraw();
		// weightsChanged();
	}
	 ...
	 * 
	 * @param child
	 *            The child control for which to get the border.
